...
pub fn validate(game: &Game, _in_progress: bool) -> status::Status {
  ...
  let positive_diagonal_status = utils::check_for_duplicates_in_set(game, &positive_diagonal, "the positive diagonal");
  let negative_diagonal_status = utils::check_for_duplicates_in_set(game, &negative_diagonal, "the negative diagonal");
